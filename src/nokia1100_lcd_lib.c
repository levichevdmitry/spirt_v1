...
{    
    ...
    CS_LCD_RESET;
    RST_LCD_RESET;
    delay_ms(20);            //20 выжидем не менее 5мс для установки генератора(менее 5 мс может неработать)
    RST_LCD_SET;
    ...
    #ifndef HW1_P
        nlcd_SendByte(CMD_LCD_MODE,0xC8); // mirror Y axis (about X axis) // заремить для экрана в контроллере Олега
    #endif
...
void nlcd_Clear(void)
{
    ...
    for(i=0;i<NLCD_X_RES*NLCD_Y_RES ;i++) nlcd_SendByte(DATA_LCD_MODE,0x00);
...
void nlcd_SendByte(char mode,unsigned char c)
{
    ...
    for(i=0; i<8; i++)
    {
        ...
        c <<= 1;
        #ifdef LCD_DELAY_ON
            delay_us(NLCD_MIN_DELAY);    // *****!!!!! 34 - Минимальная задержка, при которой работает мой LCD-контроллер
        #endif
    }    
    CS_LCD_SET;
}
...
void nlcd_Putc(unsigned char c)
{
    ...
    for (i = 0; i < 5; i++ ) {
         nlcd_SendByte(DATA_LCD_MODE, nlcd_Font[c-30][i]);  //32
    }
...
void nlcd_PutcWide(unsigned char c)
{
    unsigned char i;
    if (c>127) c=c-64;     // Переносим символы кирилицы в кодировке CP1251 в начало второй
    for ( i = 0; i < 5; i++ )  // половины таблицы ASCII (начиная с кода 0x80)
    {
        unsigned char glyph = nlcd_Font[c-30][i]; //32
...
void nlcd_GotoXY_pix(char x,char y)
{
    ...
    //nlcd_SendByte(CMD_LCD_MODE,(0xB0 | ((y / 8) & 0x0F)));      // установка адреса по Y: 0100 yyyy 
...
{
    unsigned char pict_width = picture[0];  // ширина спрайта в пикселах  
    unsigned char pict_height = picture[1]; // высота спрайта в пикселах
    ...
    { // проход построчно (по банкам)
        ...
        if (i<((NLCD_Y_RES/8)+1)) // не выводить картинку за пределами экрана
        {
            ...
            {
                if ((x+j) < NLCD_X_RES) nlcd_SendByte(DATA_LCD_MODE,picture[adr]); // не выводить картинку за пределами экрана
                adr++;
            }
        }
    }
