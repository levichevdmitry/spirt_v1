...
void StartAllConvert_T(unsigned char pin)
{
    ...
    OWI_SkipRom(pin);
    OWI_SendByte(DS18B20_CONVERT_T, pin);
}
...
float GetTemperatureSkipRom(unsigned char pin)
{
    ...
    scratchpad[0] = OWI_ReceiveByte(pin);
    scratchpad[1] = OWI_ReceiveByte(pin);
...
unsigned char InitSensor(unsigned char * romValue, unsigned char pin, signed char lowAlm, signed char hiAlm, unsigned char resolution)
{
     ...
     OWI_DetectPresence(pin);
     OWI_MatchRom(romValue, pin);
     OWI_SendByte(DS18B20_WRITE_SCRATCHPAD, pin);
     OWI_SendByte(hiAlm, pin);
     OWI_SendByte(lowAlm, pin);
     OWI_SendByte(resolution, pin);
     ...
     OWI_SendByte(DS18B20_READ_SCRATCHPAD, pin);
     for (i=0; i<8; i++) {
        scratchpad[i] = OWI_ReceiveByte(pin);
     }
